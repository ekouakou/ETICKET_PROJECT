      ...
      },
      {
        ...
        render: (data, type, row) => `
          <div class="d-flex justify-content-start">
            <button class="btn btn-warning btn-sm me-2 action-edit" data-id="${row.LG_EVEID}">
              <i class="fa fa-edit"></i>
            </button>
            <button class="btn btn-danger btn-sm me-2 action-delete" data-id="${row.LG_EVEID}">
              <i class="fa fa-trash"></i>
            </button>
            <button class="btn btn-primary btn-sm action-toggle-status" 
              data-id="${row.LG_EVEID}" 
              data-status="${row.STR_EVESTATUT}">
              <i class="fa ${row.STR_EVESTATUT === "enable" ? "fa-ban" : "fa-check"}"></i>
            </button>
          </div>
        `,
      },
