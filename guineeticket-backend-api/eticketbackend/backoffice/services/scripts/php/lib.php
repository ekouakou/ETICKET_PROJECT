...
function uploadFile($target_dir, $file, $isFileImage = true) {
    $validation = "";
    $prefixe = "";
    try {
        $prefixe = time();
//        $target_file = $target_dir . $prefixe . '-' . str_replace(" ", "", basename($file["name"])); //a decommenter en casde probleme
        $target_file = $target_dir . $prefixe . "." . strtolower(pathinfo(basename($file["name"]), PATHINFO_EXTENSION));
...
//                $validation = $prefixe . '-' . basename($file["name"]); //a decommenter en cas de probleme
                //echo "===". $validation . "====" . basename($target_file);
                $validation = basename($target_file);
            } else {
...
function readFilename($path) {
    ...
    try {
        ...
        //$fichier = 'chemin/vers/votre_fichier.txt';
// Lire le contenu du fichier
        $validation = file_get_contents($path);
    } catch (Exception $exc) {
        echo $exc->getTraceAsString();
    }
    return $validation;
